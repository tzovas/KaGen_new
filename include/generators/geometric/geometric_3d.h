...
class Geometric3D {
 ...
 protected:
  ...
  void ComputeChunk(const SInt chunk_id) {
    ComputeChunk(chunk_id, config_.n, chunks_per_dim_, chunks_per_dim_,
                 chunks_per_dim_, 0, 0, 0, 1, 0);
  }

  void ComputeChunk(const SInt chunk_id, const SInt n, const SInt row_k,
                    const SInt column_k, const SInt depth_k,
                    const SInt chunk_start_row, const SInt chunk_start_column,
                    const SInt chunk_start_depth, const SInt level, const SInt offset) {
    ...
    SInt chunk_start =
        Encode(chunk_start_column, chunk_start_row, chunk_start_depth);
    ...
    if (chunk_row < row_splitter + chunk_start_row) {
      ...
      if (chunk_column < column_splitter + chunk_start_column) {
        ...
        if (chunk_depth < depth_splitter + chunk_start_depth)
          ComputeChunk(chunk_id, z_variate, row_splitter, column_splitter,
                       depth_splitter, chunk_start_row, chunk_start_column,
                       chunk_start_depth, level + 1, offset);
        else
          ComputeChunk(chunk_id, h_variate - z_variate, row_splitter,
                       column_splitter, depth_k - depth_splitter,
                       chunk_start_row, chunk_start_column,
        ...
        if (chunk_depth < depth_splitter + chunk_start_depth)
          ...
          ComputeChunk(chunk_id, z_variate, row_splitter,
                       column_k - column_splitter, depth_splitter,
                       chunk_start_row, chunk_start_column + column_splitter,
                       chunk_start_depth, level + 1, offset + h_variate);
        else
          ComputeChunk(chunk_id, v_variate - h_variate - z_variate,
                       row_splitter, column_k - column_splitter,
                       depth_k - depth_splitter, chunk_start_row,
                       chunk_start_column + column_splitter,
      ...
      if (chunk_column < column_splitter + chunk_start_column) {
        ...
        if (chunk_depth < depth_splitter + chunk_start_depth)
          ComputeChunk(chunk_id, z_variate, row_k - row_splitter,
                       column_splitter, depth_splitter,
                       chunk_start_row + row_splitter, chunk_start_column,
                       chunk_start_depth, level + 1, offset + v_variate);
        else
          ComputeChunk(chunk_id, h_variate - z_variate, row_k - row_splitter,
                       column_splitter, depth_k - depth_splitter,
                       chunk_start_row + row_splitter, chunk_start_column,
        ...
        if (chunk_depth < depth_splitter + chunk_start_depth)
          ComputeChunk(chunk_id, z_variate, row_k - row_splitter,
                       column_k - column_splitter, depth_splitter,
                       chunk_start_row + row_splitter,
                       chunk_start_column + column_splitter, chunk_start_depth,
                       level + 1, offset + v_variate + h_variate);
        else
          ComputeChunk(chunk_id, n - v_variate - h_variate - z_variate,
                       row_k - row_splitter, column_k - column_splitter,
                       depth_k - depth_splitter, chunk_start_row + row_splitter,
                       chunk_start_column + column_splitter,
                       chunk_start_depth + depth_splitter, level + 1, offset + v_variate + h_variate + z_variate);
      }
    }
  }
  ...
  void GenerateVertices(const SInt chunk_id, const SInt cell_id, const bool neighborFlag = false) {
    ...
    if (!std::get<4>(chunk)) GenerateCells(chunk_id);
  ...
  virtual void GenerateEdges(const SInt chunk_row, const SInt chunk_column,
                             const SInt chunk_depth) = 0;
